...
def _connect_ad() -> Connection:
    ...
    if not server_uri or not base_dn:
        raise RuntimeError('Configuracao LDAP incompleta.')
...
def _dedupe_members(members: list[tuple[str, str]]) -> list[tuple[str, str]]:
    deduped: list[tuple[str, str]] = []
    seen: set[tuple[str, str]] = set()
    for name, username in members:
        key = ((username or '').strip().lower(), (name or '').strip().lower())
        if key in seen:
            continue
        seen.add(key)
        deduped.append((name, username))
    return deduped


def refresh_access_snapshot(root_path: str) -> tuple[int, int, int]:
    if not os.path.isdir(root_path):
        raise RuntimeError('Caminho das pastas nao encontrado.')
    ...
    try:
        ...
        group_members_cache: dict[str, list[tuple[str, str]]] = {}
            ...
            for folder_name, folder_path in folders:
                ...
                for group_name, level in group_levels.items():
                    ...
                    if group_is_global.get(group_name):
                        ...
                        cached_members = group_members_cache.get(group_name)
                        if cached_members is None:
                            cached_members = _dedupe_members(_resolve_group_members(conn, group_name))
                            group_members_cache[group_name] = cached_members
