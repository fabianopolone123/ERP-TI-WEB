...
ERP_MODULES = [
    ...
    {'slug': 'seguranca', 'label': 'Segurança', 'url_name': 'seguranca'},
...
def _build_security_assessment(equipment: Equipment, now_dt=None) -> dict:
    now_dt = now_dt or timezone.now()
    issues: list[str] = []
    score = 100
    priority = 3

    fw_domain = equipment.fw_domain_enabled
    fw_private = equipment.fw_private_enabled
    fw_public = equipment.fw_public_enabled
    fw_states = [fw_domain, fw_private, fw_public]

    if fw_states.count(None) == 3:
        issues.append('Sem dados de firewall')
        score -= 20
        priority = min(priority, 2)
    else:
        disabled_profiles = []
        if fw_domain is False:
            disabled_profiles.append('Domínio')
        if fw_private is False:
            disabled_profiles.append('Privado')
        if fw_public is False:
            disabled_profiles.append('Público')
        if disabled_profiles:
            issues.append(f'Firewall desativado: {", ".join(disabled_profiles)}')
            score -= 45
            priority = min(priority, 0)

    if equipment.defender_service_running is False:
        issues.append('Serviço do Defender parado')
        score -= 25
        priority = min(priority, 1)
    elif equipment.defender_service_running is None:
        issues.append('Sem dados do serviço Defender')
        score -= 10
        priority = min(priority, 2)

    if equipment.defender_realtime_enabled is False:
        issues.append('Defender sem proteção em tempo real')
        score -= 35
        priority = min(priority, 0)
    elif equipment.defender_realtime_enabled is None:
        issues.append('Sem dados de proteção em tempo real')
        score -= 10
        priority = min(priority, 2)

    if not (equipment.antivirus_names or '').strip():
        issues.append('Antivírus não identificado')
        score -= 25
        priority = min(priority, 1)

    if equipment.last_security_scan_at:
        age = now_dt - equipment.last_security_scan_at
        if age > timedelta(days=30):
            issues.append('Dados de segurança desatualizados (>30 dias)')
            score -= 25
            priority = min(priority, 1)
        elif age > timedelta(days=14):
            issues.append('Dados de segurança com mais de 14 dias')
            score -= 12
            priority = min(priority, 2)
    else:
        issues.append('Sem coleta de segurança')
        score -= 30
        priority = min(priority, 1)

    score = max(0, min(100, score))
    if score <= 40 or priority == 0:
        level = 'critical'
        level_label = 'Crítico'
        level_color = '#ef4444'
        priority = 0
    elif score <= 65 or priority == 1:
        level = 'high'
        level_label = 'Alto'
        level_color = '#f97316'
        priority = min(priority, 1)
    elif score <= 85 or priority == 2:
        level = 'medium'
        level_label = 'Médio'
        level_color = '#f59e0b'
        priority = min(priority, 2)
    ...
    return {
        'score': score,
        'issues': issues,
        'issues_text': ' | '.join(issues) if issues else 'Sem alertas',
        'level': level,
        'level_label': level_label,
        'level_color': level_color,
        'priority': priority,
        'fw_domain_label': _format_bool_status(fw_domain),
        'fw_private_label': _format_bool_status(fw_private),
        'fw_public_label': _format_bool_status(fw_public),
        'defender_service_label': _format_bool_status(equipment.defender_service_running),
        'defender_rt_label': _format_bool_status(equipment.defender_realtime_enabled),
        'antivirus_label': (equipment.antivirus_names or '').strip() or 'Sem dados',
    }
...
class SegurancaView(LoginRequiredMixin, TemplateView):
    template_name = 'core/seguranca.html'

    def post(self, request, *args, **kwargs):
        if not is_ti_user(request):
            messages.error(request, 'Apenas usuários do departamento TI podem executar varredura de segurança.')
        ...
        hosts_text = (request.POST.get('security_hosts') or '').strip()
        hosts = parse_hosts_text(hosts_text) or parse_hosts_text(_inventory_default_hosts())
        if not hosts:
            messages.error(request, 'Informe pelo menos um host para varredura (ex.: CPU-001..CPU-050).')
            return redirect('seguranca')
        ...
        if result['ok']:
            messages.success(
                request,
                f'Varredura executada em {stamp}: {result["ok"]} host(s) atualizado(s), {result["failed"]} falha(s).',
            )
        else:
            messages.error(request, f'Varredura executada em {stamp}: nenhuma máquina atualizada.')
        for line in result.get('messages', [])[:15]:
            if 'erro' in line.lower():
                messages.error(request, line)
            else:
    ...
    def get_context_data(self, **kwargs):
        ...
        is_ti = is_ti_user(self.request)
        context['is_ti_group'] = is_ti
        context['modules'] = build_modules('seguranca') if is_ti else []
        if not is_ti:
            return context

        risk_filter = (self.request.GET.get('risk') or 'all').strip().lower()
        if risk_filter not in {'all', 'critical', 'high', 'medium', 'ok'}:
            risk_filter = 'all'
        search_raw = (self.request.GET.get('q') or '').strip().lower()

        now_dt = timezone.now()
        items = list(Equipment.objects.all().order_by('hostname', 'tag_code', 'id'))
        assessed_items = []
        summary = {'total': 0, 'critical': 0, 'high': 0, 'medium': 0, 'ok': 0}

        for item in items:
            assessment = _build_security_assessment(item, now_dt=now_dt)
            item.security = assessment
            summary['total'] += 1
            summary[assessment['level']] += 1

            if risk_filter != 'all' and assessment['level'] != risk_filter:
                continue
            if search_raw:
                haystack = ' '.join(
                    [
                        ...
                        (item.sector or ''),
                        (item.model or ''),
                        assessment['issues_text'],
                        assessment['antivirus_label'],
                    ]
                ).lower()
        ...
        assessed_items.sort(
            key=lambda row: (
                row.security['priority'],
                -(row.security['score']),
                -int(row.last_security_scan_at.timestamp()) if row.last_security_scan_at else 0,
                (row.hostname or '').lower(),
                row.id,
            )
        )

        context['security_items'] = assessed_items
        context['security_summary'] = summary
        context['security_filters'] = {'risk': risk_filter, 'q': self.request.GET.get('q') or ''}
        context['inventory_default_hosts'] = _inventory_default_hosts()
        context['inventory_timeout_seconds'] = int(getattr(settings, 'INVENTORY_POWERSHELL_TIMEOUT', 120) or 120)
        return context
